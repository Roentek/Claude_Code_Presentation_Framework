...
## 2026-08-26 — Fresh-machine `setup.sh` bootstrap fixes (new computer, PATH/marketplace/CLI bugs)
- **Context:** User set up this repo on a genuinely new Windows machine by copying the whole project folder over (not a fresh `git clone` — confirmed by a stale `MorarMonil` username baked into `.claude/settings.local.json`'s `OPENSPACE_HOST_SKILL_DIRS` from the old machine, and an untracked, broken `tools/openspace/.venv` pointing at a nonexistent `C:\Program Files\Python313\python.exe`). `setup.sh` does NOT auto-install Python/Node/uv — `sys-python.sh`/`sys-node.sh`/`sys-uvx.sh` only check and warn, so a machine missing all three cascades into many downstream failures.
- **Windows UAC blocks MSI installers headlessly:** `winget install` for MSI-based packages (Node.js LTS, Python) prints "installer will request to run as administrator" and hangs forever in a non-interactive session (Bash/PowerShell tools run with stdin attached to null — no one can answer the UAC prompt). Fix: prefer non-elevated install paths — `winget install --scope user` (works for Node via portable zip extraction, no MSI); for Python, skip MSI entirely and use `uv python install 3.12` (uv manages its own standalone interpreters, zero admin, zero MSI) — then add uv's python dir to PATH.
- **A killed elevated install can leave `ERROR_INSTALL_ALREADY_RUNNING` (1618) forever:** once a UAC-stuck winget/msiexec attempt is killed, the Windows Installer global mutex may still show "Another installation already in progress" on every subsequent attempt (even for unrelated packages) — this isn't a real stuck process (msiexec.exe's persistent `/V` service host has no command line and is normal), it's an internal rollback state. Workaround: avoid MSI installers entirely (see above) rather than trying to clear the lock.
- **Registry PATH changes (`setx`, or `[Environment]::SetEnvironmentVariable(...,"User")`) do NOT propagate to child processes of an already-running session** — only brand-new top-level processes (e.g. a fresh terminal window) pick up registry env changes. Within one Claude Code session, every Bash/PowerShell tool call inherits a fixed PATH snapshot from the host process. Fix: explicitly `export PATH="...;$PATH"` (or `$env:PATH = "...;$env:PATH"`) at the top of every command that needs a tool installed earlier in the same session, in addition to the permanent `setx`/registry write (which benefits *future* sessions).
- **`winget install` on an already-installed, up-to-date package can still exit non-zero** ("Found an existing package already installed... No available upgrade found") — scripts that only check the exit code (not the actual installed state) will falsely report failure on every re-run. Fixed in `cli-stripe.sh` (added a `winget list --id ... -e` fallback check) and `sys-obsidian.sh` (fixed the install-detection path, which was checking `$LOCALAPPDATA/Obsidian` when Obsidian actually installs to `$LOCALAPPDATA/Programs/Obsidian` — so it was never detecting itself as installed, guaranteeing a redundant winget call every run).
- **`Stripe.StripeCLI` was the wrong winget package ID** in `cli-stripe.sh` — real ID is `Stripe.StripeCli` (lowercase "li"), confirmed via `winget search stripe`. Winget exact-match `-e` is case-sensitive.
- **`claude skill install <name>@<marketplace>` does not exist as a CLI subcommand** in the current Claude Code CLI (`claude --help` lists only `agents, auth, auto-mode, doctor, gateway, import, install, mcp, plugin|plugins, project, setup-token, ultrareview, update`). `lib.sh`'s `_install_skill()` helper was calling this nonexistent command, silently failing for every one of its 6 callers (all 5 `skill-apify-*.sh` scripts). **Fix:** marketplace skills install exactly like plugins — `claude plugin install <name>@<marketplace>`. Fixed `_install_skill()` in `lib.sh` to call `claude plugin install` instead; this was a long-standing bug (predates this session), not something this session introduced.
- **Marketplace registration keys are controlled by each marketplace repo's own manifest, NOT by whatever key `settings.json`'s `extraKnownMarketplaces` declares.** `mattpocock/skills`'s manifest names itself `mattpocock` (not `mattpocock-skills`); `thedotmack/claude-mem`'s manifest names itself `thedotmack` (not `claude-mem`). `claude plugin marketplace list` is the authority — check it before assuming a plugin string like `name@declared-key` will resolve. Fixed `enabledPlugins` (`claude-mem@thedotmack`, `mattpocock-skills@mattpocock`), `extraKnownMarketplaces` dict keys, and `plugin-claude-mem.sh`/`plugin-mattpocock-skills.sh` to match reality. (A previous session-audit fork flagged `claude-mem@thedotmack` as a "stale duplicate" of `claude-mem@claude-mem` based on internal settings.json consistency alone — that was wrong; always verify against `claude plugin marketplace list`, not just internal file cross-referencing.)
- **`tools/openspace/.venv` being untracked (normal — it's a venv) permanently defeated `openspace-sync.sh`'s dirty-check**, since `git status --porcelain` (no flag) counts untracked files, meaning the submodule always looked "dirty" and auto-sync silently no-opped forever once a venv existed. Fixed `openspace-sync.sh` to use `git status --porcelain --untracked-files=no` — only genuine edits to tracked files should block sync, per the established convention that the submodule carries no local edits ([[openspace-overrides-pattern]]).
- **`uv sync`/`uv pip install -e` auto-selects the newest available Python (e.g. 3.14) when a project has no `.python-version` pin and only a loose `requires-python = ">=3.10"`.** This broke `tools/lightrag-plus` (`pyiceberg==0.11.1`, pulled in transitively via `supabase`→`storage3`, publishes wheels only up to cp313 — no cp314 wheel existed, forcing a from-source build that needs MSVC Build Tools, which aren't installed and would themselves hit the UAC-hang problem). **Fix:** added `tools/lightrag-plus/.python-version` pinning `3.12` (has a wheel). For `tools/openspace` (a git submodule — can't commit a `.python-version` there without dirtying it), hardened `tool-openspace.sh` instead to explicitly `uv venv --python 3.12` before `uv pip install -e`, and to detect+recreate a broken/missing venv automatically (a stale `.venv` copied from another machine, pointing at a Python path that no longer exists, previously surfaced as a silently-ignored "Broken Python trampoline" warning that still printed `✓ installed` — now actually fixed rather than papered over).
- **`npx skills@latest add <url>` manages the ENTIRE `.claude/skills/` tree via its own `skills-lock.json`, not just the skill it's adding** — running it interactively (outside `setup.sh`) briefly showed ~35 unrelated mattpocock-skills directories as deleted in `git status`. They were still listed in the regenerated `skills-lock.json` (not actually dropped from tracking) and were fully recoverable via `git checkout --`, but this is a sharp edge worth remembering: never run `npx skills@latest add ...` interactively for a quick test without immediately checking `git status` afterward. The `docs.stripe.com` well-known skill source now publishes 8 skills (added `connect-recommend`, `connect-required-verification-information`, `stripe-apps`, `stripe-docs` beyond the original 4 documented in CLAUDE.md: `stripe-best-practices`, `stripe-directory`, `stripe-projects`, `upgrade-stripe`).
- **`_timeout 60` was too short** for `npx skills@latest add https://docs.stripe.com` on a cold cache (npx package download + well-known skill discovery + multi-target install to Claude Code/Codex/Gemini CLI/GitHub Copilot) — bumped to 120s in `setup.sh`.
- **Files touched:** `.claude/hooks/setup.sh` (Stripe skills timeout 60→120), `.claude/hooks/install/{cli-stripe.sh, sys-obsidian.sh, tool-openspace.sh, lib.sh, plugin-claude-mem.sh, plugin-mattpocock-skills.sh}`, `.claude/hooks/openspace-sync.sh`, `.claude/settings.json` (enabledPlugins + extraKnownMarketplaces keys, plus a prior-session PATH/permissions cleanup), `tools/lightrag-plus/.python-version` (new).
- **Result:** `setup.sh` reached "✓ Setup complete." with 0 errors and wrote `.claude/.setup-complete` on the third full run.
