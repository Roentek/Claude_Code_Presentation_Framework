...
- Split into multiple files when one task detects/polls for new items and another does the heavy work (API calls, LLM, posting output) — see [`trigger-api-reference.md`](../docs/trigger-api-reference.md) for the orchestrator+processor pattern
...
Use [`trigger-api-reference.md`](../docs/trigger-api-reference.md) for complete code examples: task patterns, schedules, waits, triggerAndWait, batch triggers, debounce, and schema tasks with Zod validation.
